...
void adenv::init(float sample_rate)
{
    ...
    curve_scalar_    = -5.0f; // full linear
    phase_           = 0;
    multiplier_      = 1.0f;
...
float adenv::process()
{
    ...
    switch(current_segment_)
    {
        case ADENV_SEG_ATTACK:
            ...
            {
                output_ = (static_cast<float>(phase_)
...
float adenv::transeg()
{
    float out, beg, val, i, n, curve;
    float time_samps = (uint32_t)(segment_time_[current_segment_] * sample_rate_);
    curve = curve_scalar_;
    beg   = current_segment_ == ADENV_SEG_DECAY ? 1.0f : 0.01f;
    i     = phase_;
    n     = time_samps;
    val   = output_;
    out   = beg
          + (val - beg) * (1 - expf(i * curve / (n - 1))) / (1 - expf(curve));
    return out;
}
