...
void copy_wav(const char* filename)
{
    ...
    uint32_t current_qspi_offset = System::kQspiBootloaderOffset;

    do
    {
        // This avoids attempted writes greater beyond the QSPI's address space
    ...
    // It's not strictly necessary to deinitialize the FatFS drivers,
    // but it ensures nothing will go wrong with simply removing the drive.
    fsi.DeInit();
}