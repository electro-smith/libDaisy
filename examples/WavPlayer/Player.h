...
/** Temp namespace since this will replace existing stuff within libDaisy */
namespace temp
{

/**
 * WAV file Streaming Playback
 * this class only supports streaming of  16-bit WAV Files
 *
 ...
 *
 * The number of files to load, and the maximum buffer sizes
 * are supplied as template parameters to allow for reduced
 * memory requirements based on application requirements.
 *
 ...
 * There are times, like when restarting playback that will trigger the entire
 * buffer to be filled.
 */
