...
typedef struct
{
	...
	dsy_gpio_t   cs, clk, data[SR_4021_MAX_PARALLEL];
	uint8_t		 states[8 * SR_4021_MAX_DAISYCHAIN * SR_4021_MAX_PARALLEL];
} dsy_sr_4021_handle_t;

void	dsy_sr_4021_init(dsy_sr_4021_handle_t *sr);
void dsy_sr_4021_update(dsy_sr_4021_handle_t *sr); // checks all 8 states per configured device.
