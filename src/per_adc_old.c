...
uint32_t pin_hash(dsy_gpio_pin *pin)
{
    return (uint32_t)((pin->port << 4) | pin->pin);
}

const uint32_t adc_channel_from_pin(dsy_gpio_pin *pin)
{
    uint32_t val;
    val = ((pin->port << 4) | pin);// (should be adjacent nibbles)
    // For now just a rough switch case for all ADC_CHANNEL values
    dsy_gpio_pin adcpins[DSY_ADC_PIN_LAST] = 
    {
        {DSY_GPIOA, 6}, // CHN3
        {DSY_GPIOC, 4}, // CHN4
        {DSY_GPIOB, 1}, // CHN5
    ...
    for (size_t i = 0; i < DSY_ADC_PIN_LAST; i++)
    {
...
AdcHandle::Init(AdcChannelConfig *cfg, size_t num_channels)
{
    ...
    {
        // OG
        //sConfig.Channel = dsy_adc_channel_map[adc.dsy_hadc->active_channels[i]];
        //sConfig.Rank    = dsy_adc_rank_map[i];
        
        // Some how get the ADC CHANNEL data from a map
        sConfig.Channel = adc_channel_from_pin(&config_[i].pin_config);
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
    if(hadc->Instance == ADC1)
    {
        ...
        {
            uint8_t chn              = adc.dsy_hadc->active_channels[i];
            uint8_t current_position = adc.mux_index[chn];
            ...
            if(adc.mux_channels[chn] > 0)
            {
                ...
                *adc.mux_cache[i][current_position] = adc.dma_buffer[i];
                ...
                //adc.mux_index[i] += 1;
                adc.mux_index[chn] += 1;
                // if(adc.mux_index[i] > adc.mux_channels[chn])
                if(adc.mux_index[chn] > adc.mux_channels[chn])
                {
                    adc.mux_index[chn] = 0;
                    //adc.mux_index[i] = 0;
                }
                //				write_mux_value(chn, adc.mux_index[i]);
