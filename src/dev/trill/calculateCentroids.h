...
WORD calculateCentroids(WORD *centroidBuffer,
                        WORD *sizeBuffer,
                        BYTE  maxNumCentroids,
                        BYTE  minSensor,
                        BYTE  maxSensor,
                        BYTE  numSensors)
{
    ...
    WORD lastSensorVal, currentSensorVal, currentWeightedSum,
        currentUnweightedSum;
    ...
    for(sensorIndex = 0, actualHardwareIndex = minSensor;
        sensorIndex < numSensors;
        sensorIndex++)
    {
        ...
        if(inCentroid)
        {
            ...
            if(currentSensorVal == 0)
            {
                ...
                {
                    temp = ((long)currentWeightedSum << SLIDER_BITS)
                           / currentUnweightedSum;
                    centroidBuffer[centroidIndex]
                        = (currentStart << SLIDER_BITS) + (WORD)temp;
            ...
            if(sensorIndex >= 2)
            {
                if(troughDepth > wAdjacentCentroidNoiseThreshold
                   && currentSensorVal
                          > lastSensorVal + wAdjacentCentroidNoiseThreshold)
                {
                    if(currentUnweightedSum > wMinimumCentroidSize)
                    {
                        temp = ((long)currentWeightedSum << SLIDER_BITS)
