...
class CentroidDetection
{
  public:
    ...
    CentroidDetection(){};
    CentroidDetection(unsigned int numReadings,
                      unsigned int maxNumCentroids,
                      float        sizeScale);
    CentroidDetection(const std::vector<unsigned int>& order,
                      unsigned int                     maxNumCentroids,
                      float                            sizeScale);
    int  setup(unsigned int numReadings,
               unsigned int maxNumCentroids,
               float        sizeScale);
    int  setup(const std::vector<unsigned int>& order,
               unsigned int                     maxNumCentroids,
               float                            sizeScale);
