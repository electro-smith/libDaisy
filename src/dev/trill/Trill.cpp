...
static const std::map<Trill::Device, struct TrillDefaults> trillDefaults = {
    ...
    {Trill::BAR,
     TrillDefaults("Bar", Trill::CENTROID, defaultThreshold, 0x20, 2)},
    {Trill::SQUARE,
     TrillDefaults("Square", Trill::CENTROID, defaultThreshold, 0x28, 1)},
    {Trill::CRAFT,
     TrillDefaults("Craft", Trill::DIFF, defaultThreshold, 0x30, 1)},
    {Trill::RING,
     TrillDefaults("Ring", Trill::CENTROID, defaultThreshold, 0x38, 2)},
    {Trill::HEX,
     TrillDefaults("Hex", Trill::CENTROID, defaultThreshold, 0x40, 1)},
...
static_assert(
    1 == sizeof(TrillStatusByte),
    "size and layout of TrillStatusByte must match the Trill firmware");
static_assert(
    kOffsetStatusByte + sizeof(TrillStatusByte) == kOffsetChannelData,
    "Assume that channel data is available immediately after the statusByte");
...
void Trill::updateChannelMask(uint32_t mask)
{
    ...
    numChannels = std::min(int(getDefaultNumChannels()),
                           __builtin_popcount(channelMask));
}
...
const std::string& Trill::getNameFromDevice(Device device)
{
    ...
    __catch(std::exception e) { return trillDefaults.at(Device::UNKNOWN).name; }
}
...
const std::string& Trill::getNameFromMode(Mode mode)
{
    ...
    __catch(std::exception e) { return trillModes.at(Mode::AUTO); }
}
...
#define WRITE_COMMAND_BUF(data) \
    writeCommandAndHandle(data, sizeof(data), __PRETTY_FUNCTION__)
#define WRITE_COMMAND(command) \
    writeCommandAndHandle(command, __PRETTY_FUNCTION__)
#define READ_BYTES_FROM(offset, data, size) \
    readBytesFrom(offset, data, size, __PRETTY_FUNCTION__)
#define READ_BYTE_FROM(offset, byte) \
    readBytesFrom(offset, byte, __PRETTY_FUNCTION__)
...
int Trill::writeCommandAndHandle(const i2c_char_t* data,
                                 size_t            size,
                                 const char*       name)
{
    ...
    {
        fprintf(stderr,
                "Trill: cannot write more than 3 bytes to the device\n");
        return -1;
    }
    ...
    if(ret != bytesToWrite)
    {
        fprintf(
            stderr,
            "Trill: failed to write command \"%s\"; ret: %d, errno: %d, %s.\n",
            name,
            ret,
            errno,
            strerror(errno));
    ...
    if(kCommandReset == buf[1])
        return usleep(
            500000); // it won't ack after reset ... (TODO: should it?)
    else
        return waitForAck(buf[1], name);
}

int Trill::readBytesFrom(const uint8_t offset,
                         i2c_char_t&   byte,
                         const char*   name)
{
                         ...
                         const char*   name)
{
    ...
    {
        fprintf(stderr,
                "%s: failed to read %d bytes. ret: %d\n",
                name,
                size,
                bytesRead);
    ...
    while(totalSleep < 200000)
    {
        ...
        if(kCommandAck == buf[0])
        {
            ...
            verbose&& printf("Ack'ed %d(%d) with %d %d %d\n",
                             ...
                             buf[1],
                             buf[2]);
            if(verbose && (kCommandIdentify != command)
               && (buf[1] != command || buf[2] != cmdCounter))
            {
...
#define REQUIRE_FW_AT_LEAST(num)                                          \
    ...
    {                                                                     \
        fprintf(stderr,                                                   \
                "%s unsupported with firmware version %d, requires %d\n", \
                __PRETTY_FUNCTION__,                                      \
                firmware_version_,                                        \
                num);                                                     \
        return 1;                                                         \
...
void Trill::updateRescale()
{
    enum
    {
        kRescaleFactorsComputedAtBits = 12
    };
    float scale = (1 << (16 - numBits))
                  / float(1 << (16 - kRescaleFactorsComputedAtBits));
...
int Trill::setMinimumTouchSize(float minSize)
{
    ...
    i2c_char_t buf[] = {kCommandMinimumSize,
                        (i2c_char_t)(size >> 8),
                        (i2c_char_t)(size & 0xFF)};
    return WRITE_COMMAND_BUF(buf);
}
...
int Trill::setTimerPeriod(float ms)
{
    if(firmware_version_ >= 3)
    {
        ...
        if(period > 255)
        {
            // shouldn't get here
            fprintf(stderr,
                    "Trill:setTimerPeriod(): the requested %f ms cannot be "
                    "achieved. Using %lu instead\n",
                    ms,
                    (unsigned long)(period * ticks / 32));
            period = 255;
        }
        i2c_char_t buf[]
            = {kCommandTimerPeriod, i2c_char_t(period), i2c_char_t(ticks)};
        if(WRITE_COMMAND_BUF(buf))
            return 1;
    }
    else
    {
        ...
        i2c_char_t buf[] = {kCommandAutoScanInterval,
                            (i2c_char_t)(arg >> 8),
                            (i2c_char_t)(arg & 0xFF)};
        if(WRITE_COMMAND_BUF(buf))
            return 1;
    ...
    if(firmware_version_ >= 3)
    {
        ...
        if(interval)
        {
            ...
            ScanTriggerMode mode
                = ScanTriggerMode(scanTriggerMode | kScanTriggerTimer);
...
static unsigned int bytesFromSlots(size_t numWords, size_t transmissionWidth)
{
    ...
    {
        default:
        case 16: return numWords * 2;
        case 12: return numWords + (numWords + 1) / 2;
        case 8: return numWords;
    }
}
...
int Trill::readI2C(bool shouldReadStatusByte)
{
    ...
    i2c_char_t offset
        = shouldReadStatusByte ? kOffsetStatusByte : kOffsetChannelData;
    if(READ_BYTES_FROM(offset, dataBuffer.data(), dataBuffer.size()))
    {
        num_touches_ = 0;
        fprintf(
            stderr,
            "Trill: error while reading from device %s at address %#x (%d)\n",
            getNameFromDevice(device_type_).c_str(),
            address,
            address);
...
void Trill::newData(const uint8_t* newData, size_t len, bool includesStatusByte)
{
    ...
    memcpy(dataBuffer.data(),
           newData,
           std::min(len * sizeof(newData[0]),
                    sizeof(dataBuffer[0]) * dataBuffer.size()));
    ...
    if(CENTROID != mode_)
    {
        ...
        switch(transmissionWidth)
        {
            ...
            case 16:
                ...
                {
                    rawData[i] = channelIntToFloat(
                        (src[2 * i] << 8) + src[2 * i + 1], rawRescale);
                }
                break;
            case 12:
            {
...
float Trill::getButtonValue(uint8_t button_num)
{
    ...
    return (((dataBuffer[dbOffset + 4 * MAX_TOUCH_1D_OR_2D + 2 * button_num]
              << 8)
             + dataBuffer[dbOffset + 4 * MAX_TOUCH_1D_OR_2D + 2 * button_num
                          + 1])
            & 0x0FFF)
           * rawRescale;
}
...
float Trill::touchSize(uint8_t touch_num)
{
    ...
    int size
        = dataBuffer[dbOffset + 2 * touch_num + 2 * MAX_TOUCH_1D_OR_2D] * 256;
...
float Trill::touchHorizontalLocation(uint8_t touch_num)
{
    ...
    int location
        = dataBuffer[dbOffset + 2 * touch_num + 4 * MAX_TOUCH_1D_OR_2D] * 256;
    location
        += dataBuffer[dbOffset + 2 * touch_num + 4 * MAX_TOUCH_1D_OR_2D + 1];
...
float Trill::touchHorizontalSize(uint8_t touch_num)
{
    ...
    int size
        = dataBuffer[dbOffset + 2 * touch_num + 6 * MAX_TOUCH_1D_OR_2D] * 256;
